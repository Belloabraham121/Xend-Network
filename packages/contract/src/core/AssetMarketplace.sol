    ...
    Pausable
{
        ...
        returns (uint256)
    {
        ...
        require(
            expirationTime <= block.timestamp + maxOrderDuration,
            "Expiration too far"
        );
        require(
    ...
    function fillOrder(
        ...
        if (isBuyOrder) {
            ...
            require(
                IERC20(order.asset).balanceOf(msg.sender) >= amount,
                "Insufficient asset balance"
            );
    ...
    function cancelOrder(
        uint256 orderId
    ) external override nonReentrant validOrder(orderId) {
        IDataTypes.MarketOrder storage order = orders[orderId];
    ...
    function getOrder(
        uint256 orderId
    ) external view override returns (IDataTypes.MarketOrder memory) {
        require(orderId > 0 && orderId < nextOrderId, "Invalid order ID");
        ...
        returns (IDataTypes.MarketOrder[] memory)
    {
        ...
        IDataTypes.MarketOrder[]
            memory activeOrders = new IDataTypes.MarketOrder[](
        ...
        IDataTypes.MarketOrder[] memory result = new IDataTypes.MarketOrder[](
            index
        );
    ...
    function matchOrders(
        ...
        require(
            orders[sellOrderId].seller != address(0),
            "First order must be sell order"
        );
        require(
            orders[buyOrderId].buyer != address(0),
            "Second order must be buy order"
        );
        require(
            orders[sellOrderId].asset == orders[buyOrderId].asset,
            "Asset mismatch"
        );
        ...
        returns (uint256)
    {
        return
            createOrder(
                ...
                maxPrice,
                true,
                block.timestamp + maxOrderDuration
            );
    }
    ...
    function _addToOrderBook(
        ...
        emit OrderBookUpdated(
            asset,
            buyOrders[asset].length,
            sellOrders[asset].length
        );
    }
    ...
    function _matchOrdersForAsset(address asset) internal {
        ...
        while (
            buyIndex < buyOrderIds.length && sellIndex < sellOrderIds.length
        ) {
    ...
    function cleanupExpiredOrders(address asset, uint256 maxCleanup) external {
        uint256 cleaned = 0;
        uint256[] storage assetOrderList = assetOrders[asset];

        // Iterate through asset orders and clean up expired ones
        for (
            uint256 i = 0;
            i < assetOrderList.length && cleaned < maxCleanup;
            i++
        ) {
            uint256 orderId = assetOrderList[i];
            IDataTypes.MarketOrder storage order = orders[orderId];

            // Check if order is active and expired (timestamp + maxOrderDuration < current time)
            if (
                order.isActive &&
                !order.isFilled &&
                (order.timestamp + maxOrderDuration) < block.timestamp
            ) {
                // Mark order as inactive
                order.isActive = false;

                // If it's a sell order, return locked tokens to seller
                if (order.seller != address(0) && order.buyer == address(0)) {
                ...
                _removeFromOrderBook(asset, orderId, order.buyer != address(0));

                emit OrderCancelled(
                    orderId,
                    order.seller != address(0) ? order.seller : order.buyer
                );
                cleaned++;
            }
        }
    ...
    function getUserTradingStats(
        address user
    ) external view returns (uint256 totalVolume, uint256 tradesCount) {
