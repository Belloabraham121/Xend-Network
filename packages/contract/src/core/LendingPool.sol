...
contract LendingPool is ILendingPool, Ownable, ReentrancyGuard, Pausable {
    ...
    // State variables
    mapping(address => mapping(address => IDataTypes.LendingPosition))
        public lendingPositions;
    mapping(address => mapping(address => IDataTypes.BorrowingPosition))
        public borrowingPositions;
        ...
        validAmount(amount)
    {
        ...
        IDataTypes.LendingPosition storage position = lendingPositions[user][
            asset
        ];
        ...
        validAmount(amount)
    {
        ...
        if (_getUserTotalBorrowValue(user) > 0) {
            uint256 newCollateralValue = _getUserCollateralValue(user) -
                _getAssetValue(asset, amount);
            uint256 borrowValue = _getUserTotalBorrowValue(user);
            require(
                _calculateHealthFactor(newCollateralValue, borrowValue) >=
                    MIN_HEALTH_FACTOR,
        ...
        validAmount(amount)
    {
        ...
        uint256 requiredCollateral = (borrowValue * BASIS_POINTS) /
            collateralFactors[collateral];
        ...
        // Update user position
        IDataTypes.BorrowingPosition storage position = borrowingPositions[
            user
        ][asset];
        position.amount += amount;
        position.startTime = block.timestamp;
        ...
        validAmount(amount)
    {
        ...
        uint256 newBorrowValue = currentBorrowValue +
            _getAssetValue(asset, amount);

        require(
            _calculateHealthFactor(collateralValue, newBorrowValue) >=
                MIN_HEALTH_FACTOR,
            "Insufficient collateral"
        );
    ...
    function liquidate(
        ...
        nonReentrant
        whenNotPaused
        supportedAsset(borrowAsset)
        supportedAsset(collateralAsset)
    {
        require(borrower != msg.sender, "Cannot liquidate self");
        ...
        IDataTypes.BorrowingPosition
            storage borrowPosition = borrowingPositions[borrower][borrowAsset];
        ...
        uint256 collateralValueToSeize = (repayValue *
            (BASIS_POINTS + liquidationPenalty)) / BASIS_POINTS;
        uint256 collateralAmount = _getAssetAmount(
            collateralAsset,
            collateralValueToSeize
        );

        IDataTypes.LendingPosition
            storage collateralPosition = lendingPositions[borrower][
                collateralAsset
            ];
        require(
    ...
    function calculateInterestRate(
        address asset
    ) public view supportedAsset(asset) returns (uint256) {
        if (totalDeposits[asset] == 0) return baseInterestRates[asset];

        uint256 utilizationRate = (totalBorrows[asset] * BASIS_POINTS) /
            totalDeposits[asset];
    ...
    function removeAsset(
        ...
        require(
            totalDeposits[asset] == 0 && totalBorrows[asset] == 0,
            "Asset has active positions"
        );
    ...
    function setLiquidationThreshold(
        ...
        require(
            newThreshold > collateralFactors[asset],
            "Threshold must be higher than collateral factor"
        );
    ...
    function _getUserCollateralValue(
        address user
    ) internal view returns (uint256) {
        uint256 totalCollateralValue = 0;

        // Get all assets from the asset factory
        address[] memory allAssets = assetFactory.getAllAssets();

        // Iterate through all assets to calculate total collateral value
        for (uint256 i = 0; i < allAssets.length; i++) {
            address asset = allAssets[i];

            // Check if asset is active by getting asset info
            ...
            // Skip if asset is not supported in lending pool
            if (!supportedAssets[asset]) {
                continue;
            }

            // Get user's lending position for this asset
            IDataTypes.LendingPosition memory position = lendingPositions[user][
                asset
            ];

            if (position.amount > 0) {
                // Calculate collateral value considering the collateral factor
                uint256 assetValue = _getAssetValue(asset, position.amount);
                uint256 collateralValue = (assetValue *
                    collateralFactors[asset]) / BASIS_POINTS;
    ...
    function _getUserTotalBorrowValue(
        ...
        // Iterate through all assets to calculate total borrow value
        for (uint256 i = 0; i < allAssets.length; i++) {
            address asset = allAssets[i];
            ...
            // Get user's borrowing position for this asset
            IDataTypes.BorrowingPosition memory position = borrowingPositions[
                user
            ][asset];

            if (position.amount > 0) {
                // Calculate borrow value at current market price
                uint256 borrowValue = _getAssetValue(asset, position.amount);
